...
GG: 选中的点/边/面 在相连的边上滑动
    E: z滑移中按e, 设置端点, 如果两边形状不一样,发端点所在的边为参照
    F: 设置端点后按f, 两边的端点切换
    Alt: 滑移中按住alt, 可以滑移超出边界
    c: 按一下跟住按alt一样，可以滑移出边界
Shift-V: 面滑移，选中的点整体滑移
    alt,c: 同gg
ctrl-shift-r:偏移边线并滑动

shift-w: 弯曲 bend
shift-e: 设置折痕

Ctrl-Alt-Space: 以选中物体创建一个临时坐标, gxx (两下x)就是按着临时坐标移动
F6(2.80改为F9): 前一个操作的属性调节面板, 相当于T出现的面板的下面部分
...
z: 线框视图切换
```
Shift-F/ 2.8改成shift-`了: 飞行模式, 跟打cs一样的查看方式, wasdeq控制方向
```
...
雕刻模式下，ctrl-中键 在工具栏上滑动可以调节工具栏大小
...
多个摄像机切换  在时间轴菜单 Marker->Bind Camera to Markers, 先选中相机再绑定

## 雕刻
Alt-M: 清除遮罩
Shift: 各种笔刷切换到平滑模式
Alt-B: 框选部分内容显示
G: 移动笔刷。 按住 ctrl 是沿法线方向移动。 
